...
class TestCoingeckoXMLPrice:
    ...
    def test_currency(self, api_client):
        ...
        price_default = float(xmltodict.parse(default.text)[XML_ROOT])
        price_aud = float(xmltodict.parse(in_aud.text)[XML_ROOT])
    ...
    def test_error(self, api_client):
        ...
        result = xmltodict.parse(res.text)[XML_ROOT]
